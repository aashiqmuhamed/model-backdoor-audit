...
class AutoresearchAgent(InterpLLMAgent):
    """Interp agent: prefill reasoning + RelP gradient attribution.

    Combines prefill continuation (model explains its reasoning) with
    RelP-modified gradients (cleaner attribution via LRP rules).
    """
    ...
    def _get_hf_model(self):
        """Get the underlying HuggingFace model for RelP patching."""
        return self.model.model.model

    def run_interp(self, ctx: InterpContext) -> dict:
        """Collect interpretability data from one sample.

        1. Prefill continuation for reasoning
        2. RelP gradient attribution for field importance
        """
        result = {}
        ...
        # 2. RelP gradients -> per-field importance
        ...
        try:
            ...
            # Fallback to basic gradients if RelP fails
            grad_data = self.model.get_embedding_gradients(ctx.prompt, charge=False)
